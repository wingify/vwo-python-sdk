...
class TestEndsWithOperandTests(unittest.TestCase):
    ...
    def test_stringified_float_test(self):
        dsl = '{"or":[{"custom_variable":{"eq":"wildcard(*123.456)"}}]}'
        custom_variables = {'eq': '87654123.456000000'}
        self.assertIs(self.segment_evaluator.evaluate('campaing_key', 'user_id', json.loads(dsl),
                      custom_variables), True)
    ...
    def test_stringified_float_test3(self):
        dsl = \
            '{"or":[{"custom_variable":{"eq":"wildcard(*123.4560000)"}}]}'
        custom_variables = {'eq': 98765123.456}
        self.assertIs(self.segment_evaluator.evaluate('campaing_key', 'user_id', json.loads(dsl),
                      custom_variables), True)

    def test_stringified_float_test2(self):
        dsl = '{"or":[{"custom_variable":{"eq":"wildcard(*123.0)"}}]}'
        custom_variables = {'eq': 7657123}
        self.assertIs(self.segment_evaluator.evaluate('campaing_key', 'user_id', json.loads(dsl),
                      custom_variables), True)
    ...
    def test_float_data_type_extra_decimal_zeros_test(self):
        dsl = '{"or":[{"custom_variable":{"eq":"wildcard(*123.456)"}}]}'
        custom_variables = {'eq': 765123.456}
        self.assertIs(self.segment_evaluator.evaluate('campaing_key', 'user_id', json.loads(dsl),
                      custom_variables), True)
