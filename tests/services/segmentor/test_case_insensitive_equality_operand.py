...
class TestCaseInsensitiveEqualityOperand(unittest.TestCase):

    def setUp(self):
        self.segment_evaluator = SegmentEvaluator()
        self.test_cases = segmentor_test_cases.get('case_insensitive_equality_operand')
    ...
    def test_boolean_data_type3(self):
        test_case = self.test_cases.get('boolean_data_type3')
        self.assertIs(self.segment_evaluator.evaluate(test_case.get('dsl'),
                      test_case.get('custom_variables')), test_case.get('expectation'))
