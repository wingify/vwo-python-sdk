...
class TestComplexDsl3(unittest.TestCase):

    def setUp(self):
        self.segment_evaluator = SegmentEvaluator()
        self.test_cases = segmentor_test_cases.get('complex_dsl_3')

    def test_false5(self):
        test_case = self.test_cases.get('false5')
    ...
    def test_false6(self):
        test_case = self.test_cases.get('false6')
        self.assertIs(self.segment_evaluator.evaluate(test_case.get('dsl'),
                      test_case.get('custom_variables')), test_case.get('expectation'))
