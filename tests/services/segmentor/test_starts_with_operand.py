...
class TestStartsWithOperand(unittest.TestCase):
    def setUp(self):
        self.segment_evaluator = SegmentEvaluator()
        self.test_cases = segmentor_test_cases.get("starts_with_operand")
    ...
    def test_float_data_type2(self):
        test_case = self.test_cases.get("float_data_type2")
        self.assertIs(
            self.segment_evaluator.evaluate(test_case.get("dsl"), test_case.get("custom_variables")),
            test_case.get("expectation"),
        )
